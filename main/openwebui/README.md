...
The `Dockerfile` uses the official `ghcr.io/open-webui/open-webui:main` image as its base. It exposes port `7860`, which is the default port for Open WebUI.
...
```bash
docker run -d -p 7860:7860 --name open-webui-container open-webui
```

This will start the Open WebUI container in detached mode and map port 8080 of your host to port 8080 of the container.

You can then access Open WebUI in your web browser at `http://localhost:7860`.
